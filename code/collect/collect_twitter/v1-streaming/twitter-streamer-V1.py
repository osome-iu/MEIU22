...
Author: Matthew R. DeVerna and Rachith Aiyappa
"""
...
SCRIPT_PURPOSE = (
    ...
    "delimited file of keywords to stream tweets from Twitter that include those "
    "keywords from the past day."
)
...
class Listener(StreamListener):
    ...
    def get_file_version(self, today):
        ...
        full_file_path = os.path.join(
            PATHS["data_dir_twitter"],
            f"streaming_data--{today}--{self.file_version}.json.gz",
        )
        while os.path.isfile(full_file_path):
            counter += 1
            full_file_path = os.path.join(
                PATHS["data_dir_twitter"],
                f"streaming_data--{today}--{counter}.json.gz",
            )
...
if __name__ == "__main__":
    ...
    args = parse_config_only_cl_arg("A V1 Twitter Streamer Script")
    ...
    TWITTER_CREDS = load_config(credentials_file)["TWITTER_CREDS"]

    # Define full path to log file name and create logger named 'log'
    full_log_path = os.path.join(PATHS["log_dir_twitter"], f"{start_time}_stream.log")
    logger = get_logger(log_dir=PATHS["log_dir_twitter"], full_log_path=full_log_path)
    ...
    logger.info(f"[*] DATA_DIRECTORY : {PATHS['data_dir_twitter']}")
    logger.info(f"[*] LOG_DIRECTORY  : {PATHS['log_dir_twitter']}")

    # Create data dir if it doesn't exist already
    try:
        os.makedirs(f"{PATHS['data_dir_twitter']}")
        logger.info("Succesfully created data directory...")
    except Exception as e:
        logger.info(e) 
        pass

    # Load file terms...
    filter_terms = load_keywords(PATHS["keywords_file"], logger)
